...
const InnerTextarea = React.forwardRef<HTMLTextAreaElement, JSX.IntrinsicElements['textarea'] & { label: string }>(
...
export function FloatingLabelTextArea(props: TextareaProps & { label: string }) {
	return (
		<Textarea
			slots={{ textarea: InnerTextarea }}
			slotProps={{ textarea: { label: props.label } }}
			sx={{ borderRadius: '6px' }}
			{...props}
		/>
	);
